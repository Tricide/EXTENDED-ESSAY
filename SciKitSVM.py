...
for amount in sampleAmounts:
    X_in, y_in = make_classification(n_samples = amount, n_features = 2, n_informative = 2)
for c in cValues:
    linear_svm = supportVector('linear', c, 0, 3, 0)
    run(linear_svm, X_in, y_in, "banana")
    
    for gamma in gammaValues:
        rbf_svm = supportVector('rbf', c, gamma, 3 ,0)
        run(rbf_svm, X_in, y_in, "banana")
        
        for shift in shiftValues:
            sig_svm = supportVector('sigmoid', c, gamma, 3, shift)
            run(sig_svm, X_in, y_in, "banana")
            
        for degree in degreeValues:
            polynomial_svm = supportVector('poly', c, gamma, degree, 0)
            run(polynomial_svm, X_in, y_in, "banana")
